...
import { getFormBook } from "../../slice/formbooks";
import { getAuthor } from "../../slice/authors";
...
const DetailProduct = () => {
            ...
            (async () => {
                ...
                if (payload?.data) {
                    ...
                    const supplierResponse = await dispatch(
                        getSupplier(payload?.data?.supplieres?._id)
                    );
                    setSupplier(supplierResponse?.payload?.data);
                    //Nhà xuất bản
                    const publistsResponse = await dispatch(
                        getPublish(payload?.data?.publishings?._id)
                    );
                    setPublists(publistsResponse?.payload?.data);
                    //Tác giả
                    const authorResponse = await dispatch(
                        getAuthor(payload?.data?.authors?._id)
                    );
                    setAuthors(authorResponse?.payload?.data);
                    //Hình thức bìa
                    const formbookResponse = await dispatch(
                        getFormBook(payload?.data?.formbooks?._id)
                    );
                    setFormbook(formbookResponse?.payload?.data);
                } else {
    ...
    return (
        <>
            {/* {console.log("supplierResponse", supplier)}
            {console.log("publistsResponse", publists)}
            {console.log("formbookResponse", formbook)}
            {console.log("authorResponse", authors)} */}
            <div className="">
                            ...
                            <div className="text-[14px]">
                                <div className="">
                                    <span className=" text-[#000]">
                                        Nhà cung cấp:
                                        <a
                                            className="text-[#2f80ec] font-semibold pl-[5px]"
                                            href=""
                                        >
                                            {supplier?.name}
                                        </a>
                                ...
                                <div className="">
                                    <span className=" text-[#000]">
                                        Nhà xuất bản:
                                        <a
                                            className=" font-semibold pl-[5px]"
                                            href=""
                                        >
                                            {publists?.name}
                                        </a>
                        ...
                        <div className="max-w-[100%] w-full">
                            ...
                            <table class="max-w-[100%] w-full text-[15px]">
                                ...
                                <tbody>
                                    ...
                                    <tr className="">
                                        ...
                                        <td className="w-[100%] p-[4px]">
                                            Top 100 sản phẩm Kỹ năng sống bán
                                            chạy của tháng
                                        </td>
                                    </tr>
                                </tbody>
                            ...
                            <div className="border-b-[1px] border-solid border-[#adadad] pb-[10px]">
                                <span className="text-[15px]">
                                    Giá sản phẩm trên Fahasa.com đã bao gồm thuế
                                    theo luật hiện hành. Bên cạnh đó, tuỳ vào
                                    loại sản phẩm, hình thức và địa chỉ giao
                                    hàng mà có thể phát sinh thêm chi phí khác
                                    như Phụ phí đóng gói, phí vận chuyển, phụ
                                    phí hàng cồng kềnh,...
                                </span>
                            </div>
